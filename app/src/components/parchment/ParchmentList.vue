<template>
  <div class="p-4">
    <div class="flex items-center justify-between">
      <h1 class="text-3xl my-4">Parchment List</h1>

      <router-link
        :to="{ name: 'ParchmentCreate' }"
        ...
        <tbody>
          <tr v-for="item in items" :key="item['@id']" class="border-b">
            <td class="px-6 py-4 text-sm">
              <router-link
                v-if="item"
                :to="{ name: 'ParchmentShow', params: { id: item['@id'] } }"
...
import { useParchmentListStore } from "@/stores/parchment/list";
...
const parchmentListStore = useParchmentListStore();
const { items, error, view, isLoading } = storeToRefs(parchmentListStore);

useMercureList({ store: parchmentListStore, deleteStore: parchmentDeleteStore });

watch(
  () => route.query.page,
  (newPage) => {
    const page = newPage as string;
    parchmentListStore.getItems(page);
  },
  { immediate: true }
);
